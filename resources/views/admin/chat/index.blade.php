...
@endsection
@section('css')
    <link rel="stylesheet" type="text/css" href="{{ assetURL('css_v2/chat.css') }}">
@endsection

@section('content')
    <div class="chat-main-row card-bg px-4">
        <div class="menu-ul-chat d-flex align-items-center justify-content-between w-100 mb-3">
            <p class="fs-24 fw-medium text-blue mb-0"> {{ __('translation.chats') }}
                (<span class="total-chats-count">{{ $chats_count }}</span>)
            </p>
                            ...
                            <li class="li-search position-sticky top-0 z-1">
                                <fieldset class="position-relative mb-2 pb-1">
                                    ...
                                    <input type="search" class="form-control" id="inputSearchTable" autocomplete="off"
                                        placeholder="{{ __('translation.Search') }}" aria-label="Search input"
                        ...
                        @if ($chats)
                            @foreach ($chats as $chat)
                                @if ($chat['other_user_id'])
                                    <li class="{{ $chat['unread_message_count'] > 0 ? 'unread' : '' }}">
                                        <a href="#" onclick="getChat(this)"
                                            ...
                                            <span class="chat-avatar-sm user-img"><img
                                                    src="{{ imageUrl($chat['other_user']['avatar_url']) }}" alt=""
                                                    class="rounded-circle"><span
                                                    class="status {{ $chat['other_user']['active_now'] ? 'online' : 'offline' }}">
                                                </span></span>
                                            <div class="badge-text-mess d-flex gap-2 flex-column">
                                                ...
                                                <span
                                                    class="fs-12 text-brown fw-normal">{{ $chat['created_since'] }}</span>
                                            </div>
                                            @if ($chat['unread_message_count'])
                                                <span
                                                    class="badge rounded-pill bg-gold ms-auto">{{ $chat['unread_message_count'] }}</span>
                                            @endif
                                        </a>
                                    </li>
                                @endif
                    ...
                    <div class="chat-window">
                                    ...
                                    @if ($lastUserId)
                                        <div class="chat-box">
                                            <div class="chats" id="chat-container"
                                                data-channel="channel_{{ $lastUserId }}">
                                                @foreach ($chatsGroupedByDate as $date => $chatGroup)
                                                    <div class="chat-line">
                                                        <span
                                                            class="chat-date">{{ date('Y/m/d', strtotime($date)) }}</span>
                                                    </div>

                                                    @foreach ($chatGroup as $chat)
                                                        @if ($chat->sender->user_type != App\Enums\UserTypesEnum::USER->value)
                                                            <div class="chat chat-right">
                                                                <div class="chat-avatar">
                                                                    <a class="avatar">
                                                                        <img alt="Jennifer Robinson"
                                                                            src="{{ imageUrl($chat->sender->avatar_url) }}"
                                                                ...
                                                                <div class="chat-body">
                                                                    <div class="chat-bubble">
                                                                        <div class="chat-content d-flex gap-2">
                                                                            ...
                                                                            <span
                                                                                class="chat-time">{{ $chat->created_at->format('H:i') }}
                                                                            </span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                    ...
                                                                    <div class="chat-bubble">
                                                                        <div class="chat-content d-flex gap-2">
                                                                            ...
                                                                            <span class="chat-time">
                                                                                <span
                                                                                    class="seen-message {{ $chat->is_read ? 'active' : '' }}"></span>
                        ...
                        @if (isset($chat))
                            <input type="hidden" name="sender_id" id="sender_id"
                                value="{{ $chat['sender_id'] == user_id() ? $chat['receiver_id'] : $chat['sender_id'] }}">
                            <input type="hidden" name="admin_id" value="{{ user_id() }}" id="admin_id">
                            <input type="hidden" name="receiver_id"
                                value="{{ $chat['receiver_id'] == user_id() ? $chat['sender_id'] : $chat['receiver_id'] }}">
                        @endif
...
@section('script')
    ...
    <script>
        var userId = {{ user_id() }}; // This will make the userId available in your JavaScript
        var authUserId = {{ user_id() }}; // This will make the userId available in your JavaScript
    </script>

    <!-- Script that defines the getChat function -->
    <script>
        var channel_from_get = '{{ request()->channel }}';
        ...
        function updateChatBox(chats) {
            ...
            for (let date in chats) {
                ...
                chatGroup.forEach(function(chat) {
                    ...
                    let authUserId = {{ auth()->id() }}; // Get the authenticated user's ID
    ...
    <script type="module">
        ...
        function addMessage(message, sender, senderName) {
            ...
            let messType = '';
            let imgUrl = "{{ imageUrl(user()->avatar_url) }}";
            let imgBox = "";
    ...
    <script>
        ...
        // `;
            ...
            $(channelSelector).find('.text-brown').text("{{ __('translation.just_now') }}");
